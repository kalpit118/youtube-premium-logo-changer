/**
 * YouTube Premium Logo Changer - Content Script
 * Uses official YouTube Premium SVG vector paths for 'Premium'.
 * Configures viewBox='0 0 101 20' & width='101' for exact alignment.
 */
...
(function () {
  ...
  const originalPathsMap = new WeakMap();
  ...
  // Generate SVG markup
  function getReplacementMarkup(text) {
    if (text.trim().toLowerCase() === 'premium') {
      return `<g id="yt-premium-replaced-paths" fill="currentColor">${OFFICIAL_PREMIUM_SVG_PATHS}</g>`;
    }

    const escaped = text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    return `
      <g id="yt-premium-replaced-paths" fill="currentColor">
        <text x="32" y="15.2" 
              ...
              fill="currentColor">${escaped}</text>
      </g>
    `;
  }

  function applyLogoTransformation() {
    if (isUpdating) return;

    // Target specifically YouTube topbar header logo
    const logoRenderer = document.querySelector('ytd-topbar-logo-renderer, a#logo, #logo-icon');
    if (!logoRenderer) return;

    // Find YouTube text paths group (id contains youtube-paths)
    const targetGroup = logoRenderer.querySelector('g[id*="youtube-paths"], #youtube-paths');
    if (!targetGroup) return;

    // Adjust SVG container viewBox & overflow for exact layout
    if (targetGroup.ownerSVGElement) {
      const svg = targetGroup.ownerSVGElement;
    ...
    // Save original paths once for restoring when disabled
    if (!originalPathsMap.has(targetGroup)) {
      originalPathsMap.set(targetGroup, targetGroup.innerHTML);
    }

    const currentTag = targetGroup.getAttribute('data-yt-premium-text');

    if (!config.enabled) {
      if (originalPathsMap.has(targetGroup) && currentTag !== 'OFF') {
        isUpdating = true;
        targetGroup.innerHTML = originalPathsMap.get(targetGroup);
        targetGroup.setAttribute('data-yt-premium-text', 'OFF');
        if (targetGroup.ownerSVGElement && targetGroup.ownerSVGElement.hasAttribute('data-original-viewbox')) {
          const origBox = targetGroup.ownerSVGElement.getAttribute('data-original-viewbox');
          if (origBox) targetGroup.ownerSVGElement.setAttribute('viewBox', origBox);
        }
        isUpdating = false;
      }
      return;
    ...
    if (currentTag === config.text && targetGroup.querySelector('#yt-premium-replaced-paths')) {
      return;
    }

    isUpdating = true;
    targetGroup.innerHTML = getReplacementMarkup(config.text);
    targetGroup.setAttribute('data-yt-premium-text', config.text);
    isUpdating = false;
  }

  // Observer targeting topbar header container
  function initObserver() {
    const container = document.querySelector('ytd-topbar-logo-renderer') || document.body;
    if (!container) return;

    const observer = new MutationObserver(() => {
      if (!isUpdating) {
        applyLogoTransformation();
      }
    });
  ...
  function loadSettings() {
    if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
      chrome.storage.local.get(['enabled', 'text'], (result) => {
        ...
        if (result.text !== undefined) config.text = result.text;
        applyLogoTransformation();
      });
    } else {
      applyLogoTransformation();
  ...
  // Storage listener for popup updates
  if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.onChanged) {
    chrome.storage.onChanged.addListener((changes, namespace) => {
      if (namespace === 'local') {
        if (changes.enabled) config.enabled = changes.enabled.newValue;
        if (changes.text) config.text = changes.text.newValue;
        applyLogoTransformation();
      }
    });
  }
  ...
  ['yt-navigate-finish', 'yt-page-data-updated', 'spfdone'].forEach(evt => {
    window.addEventListener(evt, applyLogoTransformation);
    document.addEventListener(evt, applyLogoTransformation);
  });
